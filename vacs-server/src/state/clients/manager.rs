...
impl ClientManager {
    ...
    pub async fn coverage_snapshot(&self) -> CoverageSnapshot {
        ...
        let mut positions: Vec<PositionCoverage> = online_positions
            ...
            .map(|(pid, cids, vatsim_only)| PositionCoverage {
                ...
                controller_ids: cids.iter().cloned().collect(),
                vatsim_only,
            })
            .collect();
        ...
        let mut stations: Vec<StationCoverage> = online_stations
            ...
            .map(|(sid, pid)| {
                let (controller_ids, vatsim_only) = vatsim_only
                    .get(pid)
                    .map(|cids| (cids.iter().cloned().collect(), true))
                    .unwrap_or_else(|| {
                        let cids = online_positions
                            .get(pid)
    ...
    /// Returns coverage info for a single station, or `None` if the station is not currently online.
...
pub struct PositionCoverage {
    ...
    pub controller_ids: BTreeSet<ClientId>,
